...
_re_status = _re_mod.compile(r'(?im)^\s*STATUS\s*:.*$')
import os
import socket
import time
import urllib.error
...
def main():
    ...
    for a in reversed(analyses):
        if a.get("llm_analysis"):
            ...
            break
    # The file's own docstring says the model is "useful for prose and
    # unreliable for arithmetic". Letting it own the STATUS line broke that
    # rule: it wrote HEALTHY over an open leak. Strip any status verdict the
    # model invented; the computed one is prepended below.
    summary = _re_status.sub("", summary).strip()

    # Standing faults: conditions the notification gate is muting because it
    # already told the operator once. They must appear once a day, or
    # "say it once" quietly becomes "forget about it".
    standing = []
    try:
        import alert_state as _as
        with open(os.path.join(BASE, "logs/alert_gate_state.json")) as _f:
            for _k, _v in json.load(_f).items():
                _s = _as.standing(_v)
                if _s:
                    _age = ("%.0fh" % _s["age_hours"]) if _s["age_hours"] else "?"
                    standing.append("- %s (open %s, %d muted)"
                                    % (_k.split("|")[-1], _age, _s["suppressed"]))
    except Exception:
        standing = None          # could not read != nothing open

    # The 07:00 digest on 2026-08-11 announced "STATUS: HEALTHY - All systems
    # are nominal" on a day carrying a CONFIRMED level-3 tunnel leak and a
    # deadbolt wedged for 281 consecutive checks. The status word was written
    # by the 3B narrative model, which was never told what was broken - and
    # this flag only ever considered tunnels, TCL and reboots. A digest that
    # can say "healthy" while faults stand is worse than no digest: it sends
    # the operator away from a real problem.
    healthy = ((lb == "11") and not tcl_issues and reboots == 0
               and not standing and pending == 0)
    ...
    if standing:
        fields.append({"name": "STANDING FAULTS (muted, still open)",
                       "value": "\n".join(standing)[:1000]})
    elif standing is None:
        fields.append({"name": "STANDING FAULTS",
                       "value": "not verified - could not read the gate state"})
    if tcl_issues:

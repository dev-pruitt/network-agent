...
IPTSAVE=$(command -v iptables-save || echo /usr/sbin/iptables-save)
cat > "$TMP" <<EOF
...
# Read-only. -S lists rules and cannot modify one. The watchdog needs it to
# check that Tailscale's ts-forward / ts-postrouting chains still exist;
# without it the forwarding check goes inert and a missing chain - route
# advertised but unable to carry traffic - sits there unnoticed.
#
# This line lives HERE, in the script that rewrites the whole file, because
# it used to be appended by grant-iptables-read.sh and this script silently
# deleted it on every run. One file, one complete definition.
$TARGET_USER ALL=(root) NOPASSWD: $IPTABLES -S
# iptables-save dumps EVERY table in one read-only call. Needed because
# ts-postrouting lives in nat, and sudo argument matching means a grant
# for 'iptables -S' does NOT permit 'iptables -S -t nat'.
$TARGET_USER ALL=(root) NOPASSWD: $IPTSAVE
EOF
